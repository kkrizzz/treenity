...
function Checkbox(props: { checked: boolean; styleConfig: any }) {
...
function isRequiredArgument(arg: any): boolean {
  return isNonNullType(arg.type) && arg.defaultValue === undefined;
}

function unwrapOutputType(outputType: any) {
...
function unwrapInputType(inputType: any): any {
...
function coerceArgValue(argType: any, value: string | any): any {
...
class InputArgView extends React.PureComponent<any, {}> {
  _previousArgSelection: any;
  _getArgSelection = () => {
  ...
  _setArgValue = (event, options: any) => {
...
export function defaultValue(argType: any): any {
...
function defaultGetDefaultScalarArgValue(parentField: any, arg: any, argType: any): any {
  return defaultValue(argType);
}

class ArgView extends React.PureComponent<any, any> {
  _previousArgSelection: any;
  _getArgSelection = () => {
    const { selection } = this.props;
  ...
  _removeArg = (commit: boolean): any | null => {
    const { selection } = this.props;
    const argSelection = this._getArgSelection();
    this._previousArgSelection = argSelection;
  ...
  _addArg = (commit: boolean): any | null => {
  ...
  _setArgValue = (event: any, options: any) => {
    let settingToNull = false;
    let settingToVariable = false;
    let settingToLiteralValue = false;
    try {
  ...
  _setArgFields = (fields, commit: boolean): any => {
    const { selection } = this.props;
    const argSelection = this._getArgSelection();
    if (!argSelection) {
      console.error('missing arg selection when setting arg value');
...
class ScalarInput extends React.PureComponent<any, {}> {
  _ref: any;
  _handleChange = (event) => {
...
class AbstractArgView extends React.PureComponent<any> {
  ...
  render() {
    ...
    const variablize = () => {
      ...
      */
      ...
      if (variable) {
        const newDoc: any = this.props.setArgValue(variable, false);
        ...
        if (newDoc) {
          ...
          const newVariableDefinitions: Array<any> = [
    ...
    const devariablize = () => {
      ...
      const newDoc: any = this.props.setArgValue(defaultValue, {
        commit: false,
      });

      if (newDoc) {
        const targetOperation: any = newDoc.definitions.find(
...
class AbstractView extends React.PureComponent<any, {}> {
  ...
  _modifyChildSelections = (selections: any, options: any): any | null => {
    const thisSelection = this._getSelection();
...
class FragmentView extends React.PureComponent<any, {}> {
  _previousSelection: any;
  _addFragment = () => {
    this.props.modifySelections([
      ...this.props.selections,
...
function defaultArgs(getDefaultScalarArgValue: any, makeDefaultArg: any, field: any): Array<any> {
  const args = [];
...
class FieldView extends React.PureComponent<any> {
  state = { displayFieldActions: false };

  _previousSelection: any;
  _addAllFieldsToSelections = (rawSubfields) => {
    const subFields: Array<any> = !!rawSubfields
  ...
  _setArguments = (argumentNodes: any, options: any): any | null => {
    const selection = this._getSelection();
    if (!selection) {
          ...
          {!this.state.displayFieldActions ? null : (
            <button
              ...
              onClick={(event) => {
                ...
                const conflictingNameCount = (applicableFragments || []).filter((fragment: any) => {
                ...
                const newFragmentDefinition: any = {
...
function parseQuery(text: string): any | Error {
  try {
    if (!text.trim()) {
      return null;
    }
...
let parseQueryMemoize: any = null;
function memoizeParseQuery(query: string): any {
...
class RootView extends React.PureComponent<any> {
  ...
  _previousOperationDef: any;

  _modifySelections = (selections: any, options: any): any => {
    let operationDef: any = this.props.definition;

    if (operationDef.selectionSet.selections.length === 0 && this._previousOperationDef) {
      operationDef = this._previousOperationDef;
    }

    let newOperationDef: any;
...
class Explorer extends React.PureComponent<any, any> {
  static defaultProps = {
  ...
  _setAddOperationType = (value: any) => {
  ...
  render() {
    ...
    const parsedQuery: any = memoizeParseQuery(query);
    ...
    const renameOperation = (targetOperation, name) => {
      const newName =
        name == null || name === '' ? null : { kind: 'Name', value: name, loc: undefined };
    ...
    const cloneOperation = (targetOperation: any) => {
      let kind;
    ...
    const availableFragments: any = relevantOperations.reduce((acc, operation) => {
    ...
    return (
      ...
      >
        ...
        >
          {relevantOperations.map((operation: any, index) => {
            ...
            const onCommit = (parsedDocument: any) => {
            ...
            return (
              <RootView
                ...
                onCommit={onCommit}
                onEdit={(newDefinition: any, options: any): any => {
                  let commit;
...
class ExplorerWrapper extends React.PureComponent<any, {}> {
