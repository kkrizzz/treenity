...
export function makeUrl(address: string, name: string, context: string): string {
  return `sol:${address}/${context.replace(' ', '-')}/${name}`; //(await derivedKey(address, context, name))[0].toBase58();
}
